...
```{r}
devtools::install_github("https://github.com/geeky33/substances")
```
...
The package ensures correctness by preventing invalid operations.

```{r}
# Attempting to add different substances
try(glucose + ethanol)  # Error: Cannot add different substances

# Attempting to apply logarithm to a substance
try(log(water))  # Error: Logarithm not applicable to substances
```

## Database of Common Substances

The package provides a predefined database of common substances.

```{r}
# Retrieve predefined substances
db <- get_substances_db()
db$water  # Output: Substances object for water
```
